...
app.MapGet("/health/ready", (SnapshotRepository repo) => repo.IsReady() ? Results.Ok(new { status = "ready" }) : Results.Problem(statusCode: 503, title: "Snapshot unavailable", extensions: new Dictionary<string, object?> { ["code"] = "snapshot_unavailable" }));
...
{
    eik = eik.Trim();
    if (!Eik.IsValid(eik)) return Invalid(http, "invalid_eik", "Enter a valid EIK.");
    var value = repo.GetSupplier(eik);
    return value is null ? Missing(http, "supplier_not_found", "No supplier matched this EIK in the current data snapshot.") : WithCache(http, value);
});
api.MapGet("/suppliers/{eik}/signals/{signalKey}", (string eik, string signalKey, HttpContext http, SnapshotRepository repo) =>
{
    ...
    return value is null ? Missing(http, "signal_not_found", "Signal not found in this snapshot.") : WithCache(http, value);
});
api.MapGet("/suppliers/{eik}/records/{recordId}", (string eik, string recordId, HttpContext http, SnapshotRepository repo) =>
{
    ...
    return value is null ? Missing(http, "record_not_found", "Source Record not found in this snapshot.") : WithCache(http, value);
});
...
static IResult Invalid(HttpContext http, string code, string detail) => Results.Problem(statusCode: 400, title: "Invalid request", detail: detail, extensions: new Dictionary<string, object?> { ["code"] = code, ["traceId"] = http.TraceIdentifier });
static IResult Missing(HttpContext http, string code, string detail) => Results.Problem(statusCode: 404, title: "Not found", detail: detail, extensions: new Dictionary<string, object?> { ["code"] = code, ["traceId"] = http.TraceIdentifier });
